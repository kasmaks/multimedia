...
public class ChessBoardManager : MonoBehaviour
{
	...
	public void MoveChessPiece(int x, int y)
	{
		...
		if (allowedMoves != null && allowedMoves[x, y] && selectedPiece != null)
		{
		...
		if(isCheck)
		{
			...
			foreach(ChessPiece checkPiece in Pieces)
			{
				if (checkPiece != null && checkPiece.isWhite == isWhiteTurn)
				{
					tempAllowedMoves = checkPiece.IsLegalMove();
					for(int i = 0; i< tempAllowedMoves.GetLength(0); i++)
					{
						for(int j = 0; j<tempAllowedMoves.GetLength(1); j++)
						{
							if(tempAllowedMoves[i,j] == true)
							{
								savedX = checkPiece.PositionX;
								...
								allowedMovesAfterMove = checkingPiece.IsLegalMove();
								...
								if (checkingPiece.PositionX == checkPiece.PositionX && checkingPiece.PositionY == checkPiece.PositionY)
								{
									currentlyPossibleMoves[i, j] = true;
									figureUnableToProtect = false;
								}

								if (figureUnableToProtect)
								{
								...
								{
									checkPiece.SetPossibleToMoveInCheck();
	...
	public string CheckWinner(bool turn)
	{
		...
		else
		{
			...
			manager.AddScore(new Score("1", darkPlayer, whitePlayer, now.ToString("MM/dd/yyyy H:mm"), numberOfMoves, chessNotation));
			numberOfMoves = 0;
	...
	private void FilterByPin(bool [,] allowed, ChessPiece piece, int posX, int posY)
	{
		bool checkFlag;
		if (allowed == null) {
			return ;
